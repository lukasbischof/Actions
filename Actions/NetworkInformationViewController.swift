...
class NetworkInformationViewController: NSViewController, NSTableViewDelegate, NSTableViewDataSource {
    ...
    override func viewDidLoad() {
        ...
        tableView.register(forDraggedTypes: ["drag"])
    }
    ...
    func tableView(_ tableView: NSTableView, viewFor tableColumn: NSTableColumn?, row: Int) -> NSView? {
        if let tableColumn = tableColumn {
            ...
            let tableCellView = tableView.make(withIdentifier: "networkCell\(columnIndex + 1)", owner: self) as! NSTableCellView
    ...
    func tableView(_ tableView: NSTableView, writeRowsWith rowIndexes: IndexSet, to pboard: NSPasteboard) -> Bool {
        ...
        pboard.setData(data, forType: "drag")
    ...
    func tableView(_ tableView: NSTableView, validateDrop info: NSDraggingInfo, proposedRow row: Int, proposedDropOperation dropOperation: NSTableViewDropOperation) -> NSDragOperation {
        ...
        if types.contains("drag") && dropOperation == .above {
            return .move
        } else {
            return NSDragOperation()
        }
    ...
    func tableView(_ tableView: NSTableView, acceptDrop info: NSDraggingInfo, row: Int, dropOperation: NSTableViewDropOperation) -> Bool {
        if let types = info.draggingPasteboard().types {
            if types.contains("drag") && dropOperation == .above {
                if let indexData = info.draggingPasteboard().data(forType: "drag") {
                    if let indexes = NSKeyedUnarchiver.unarchiveObject(with: indexData) as? IndexSet {
                        for index in indexes {
                            do {
                                try SettingsKVStore.sharedStore.networkingSettings.moveOptionAtIndex(UInt(index), toIndex: UInt(row))
