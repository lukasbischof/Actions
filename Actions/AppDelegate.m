...
#       endif
    ...
    NSPredicate *predicate = [NSPredicate predicateWithFormat:@"(NOT path BEGINSWITH[cd] '.') AND (path ENDSWITH '.scpt' OR path ENDSWITH '.workflow' OR path ENDSWITH '.actn' OR pathExtension = '')"];
...
- (NSString *)removeFileExtension:(NSString *)path
{
    ...
    NSString *const delemitter = @".";
    NSMutableArray *components = [[path componentsSeparatedByString:delemitter] mutableCopy];
    
    if (components.count == 1)
        return path;
    else {
        [components removeObjectAtIndex:components.count - 1];
        return [components componentsJoinedByString:delemitter];
    }
}
...
- (void)addMenuItemsForNetworkingOption:(NetworkingOption *_Nonnull)option intoMenu:(NSMenu *__autoreleasing _Nonnull *_Null_unspecified)menu
{
    ...
    switch (error) {
        case NSNetServicesTimeoutError:
        {
            NSDictionary *dict = __dict(
                NSLocalizedDescriptionKey: SWF(@"Can't resolve the service %@: timeout reached", sender.name),
        ...
        case NSNetServicesActivityInProgress:
        {
            perr(
                 ...
                 NSLocalizedDescriptionKey: @"The request cannot be processed at this time. No additional information about the network state is known.",
        ...
        case NSNetServicesInvalidError:
        {
            perr(
                 ...
                 NSLocalizedDescriptionKey: @"The service, to which you wanted to connect, was improperly configured",
                 NSLocalizedRecoverySuggestionErrorKey: @"Try a different service"
            );
        }
            break;
        ...
        case NSNetServicesUnknownError:
        {
            NSDictionary *dict = __dict(
                 ...
                 NSLocalizedRecoverySuggestionErrorKey: @"Try again or choose a different service"
            );
        ...
        case NSNetServicesNotFoundError:
        {
            perr(
                 ...
                 NSLocalizedDescriptionKey: @"The service couldn't be found on the network",
                 NSLocalizedRecoverySuggestionErrorKey: @"Check your network connection and the service's network connection. Try to reconnect if the service was disconnected from the network"
            );
        }
            break;
        ...
        case NSNetServicesCancelledError:
        {
            NSDictionary *dict = __dict(
                 ...
                 NSLocalizedDescriptionKey: @"The action was cancelled by the client"
            );
        ...
        case NSNetServicesBadArgumentError:
        {
            NSDictionary *dict = __dict(
                 ...
                 NSLocalizedRecoverySuggestionErrorKey: @"Please report this bug to the developer. The description, error code and internal debug info will be submitted, but no personal information. Thank you very much :)"
            );
        ...
        default:
        {
            NSDictionary *dict = __dict(
                 ...
                 NSLocalizedDescriptionKey: @"An error occurred, which hasn't been expected by the application",
                 NSLocalizedRecoverySuggestionErrorKey: @"Please report this bug to the developer using description and error code. Thank you very much :)"
            );
...
#warning HTTPS koennte bei fremden Clients wegen selbst-signiertem Zertifikat fehlschlagen
    NSURL *url = [NSURL URLWithString:@"https://aseider.pf-control.de/support/error/report/?appl=actions"];
    [LBFastHTTPRequest sendRequestWithURL:url httpMethod:@"POST" httpBody:reportData andResponse:^(NSData *data, NSHTTPURLResponse *httpResponse) {
        if (!data || !httpResponse) {
            if (completionHandler)
                completionHandler(NO);
            
            return; // Will be logged by LBFastHTTPRequest
        }
...
- (NSData *_Nonnull)dataFromError:(NSError *)error withUserInfo:(NSDictionary<NSString *, NSObject *> *_Nullable)userInfo
{
    ...
    if (userInfo) {
        [dict setObject:userInfo forKey:kUserInfoKey];
    }
